...
class MockHandler implements \Countable
{
    ...
    public function __invoke(\DeliciousBrains\WP_Offload_Media\Gcp\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        $response = $response instanceof \Exception ? \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\rejection_for($response) : \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\promise_for($response);
    ...
    public function append()
    {
        foreach (func_get_args() as $value) {
            if ($value instanceof ResponseInterface || $value instanceof \Exception || $value instanceof PromiseInterface || is_callable($value)) {
                ...
                throw new \InvalidArgumentException('Expected a response or ' . 'exception. Found ' . \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\describe_type($value));
            }
        }
    }
    /**
