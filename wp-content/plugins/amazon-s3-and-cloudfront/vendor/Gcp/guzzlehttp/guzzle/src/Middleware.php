...
use DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Cookie\CookieJarInterface;
...
final class Middleware
{
        ...
        return function (callable $handler) {
            return function ($request, array $options) use($handler) {
                ...
                return $handler($request, $options)->then(function (\DeliciousBrains\WP_Offload_Media\Gcp\Psr\Http\Message\ResponseInterface $response) use($request) {
                    ...
                    throw \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Exception\RequestException::create($request, $response);
                });
            };
        };
    }
        ...
        return function (callable $handler) use(&$container) {
            return function ($request, array $options) use($handler, &$container) {
                return $handler($request, $options)->then(function ($value) use($request, &$container, $options) {
                    ...
                    return \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\rejection_for($reason);
                });
            };
        };
    }
    ...
    {
        return function (callable $handler) {
            return new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\RedirectMiddleware($handler);
        };
    }
    /**
    ...
    public static function retry(callable $decider, callable $delay = null)
    {
        return function (callable $handler) use($decider, $delay) {
            return new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\RetryMiddleware($decider, $handler, $delay);
        };
    }
    /**
    ...
    {
        return function (callable $handler) {
            return new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\PrepareBodyMiddleware($handler);
        };
    }
    /**
