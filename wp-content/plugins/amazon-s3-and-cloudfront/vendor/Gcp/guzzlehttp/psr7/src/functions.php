 ...
 * @return string
 */
function str(\DeliciousBrains\WP_Offload_Media\Gcp\Psr\Http\Message\MessageInterface $message)
{
 ...
 */
function modify_request(\DeliciousBrains\WP_Offload_Media\Gcp\Psr\Http\Message\RequestInterface $request, array $changes)
{
    if (!$changes) {
        return $request;
    }
 ...
 */
function rewind_body(\DeliciousBrains\WP_Offload_Media\Gcp\Psr\Http\Message\MessageInterface $message)
{
 ...
 */
function copy_to_string(\DeliciousBrains\WP_Offload_Media\Gcp\Psr\Http\Message\StreamInterface $stream, $maxLen = -1)
{
    $buffer = '';
    if ($maxLen === -1) {
        while (!$stream->eof()) {
 ...
 * @throws \RuntimeException on error.
 */
function copy_to_stream(\DeliciousBrains\WP_Offload_Media\Gcp\Psr\Http\Message\StreamInterface $source, \DeliciousBrains\WP_Offload_Media\Gcp\Psr\Http\Message\StreamInterface $dest, $maxLen = -1)
{
    $bufferSize = 8192;
    if ($maxLen === -1) {
        while (!$source->eof()) {
 ...
 * @throws \RuntimeException on error.
 */
function hash(\DeliciousBrains\WP_Offload_Media\Gcp\Psr\Http\Message\StreamInterface $stream, $algo, $rawOutput = false)
{
    $pos = $stream->tell();
    if ($pos > 0) {
        $stream->rewind();
 ...
 * @return string
 */
function readline(\DeliciousBrains\WP_Offload_Media\Gcp\Psr\Http\Message\StreamInterface $stream, $maxLength = null)
{
    $buffer = '';
    $size = 0;
    while (!$stream->eof()) {
...
function parse_request($message)
{
    ...
    $request = new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Psr7\Request($parts[0], $matches[1] === '/' ? _parse_request_uri($parts[1], $data['headers']) : $parts[1], $data['headers'], $data['body'], $version);
...
function parse_response($message)
{
    ...
    $parts = explode(' ', $data['start-line'], 3);
    return new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Psr7\Response($parts[1], $data['headers'], $data['body'], explode('/', $parts[0])[1], isset($parts[2]) ? $parts[2] : null);
}
/**
...
function _parse_message($message)
{
    ...
    if (preg_match("/(?:^HTTP\\/|^[A-Z]+ \\S+ HTTP\\/)(\\d+(?:\\.\\d+)?)/i", $startLine, $matches) && $matches[1] === '1.0') {
        ...
        $rawHeaders = preg_replace(\DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Psr7\Rfc7230::HEADER_FOLD_REGEX, ' ', $rawHeaders);
    }
    /** @var array[] $headerLines */
    $count = preg_match_all(\DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Psr7\Rfc7230::HEADER_REGEX, $rawHeaders, $headerLines, PREG_SET_ORDER);
    // If these aren't the same, then one line didn't match and there's an invalid header.
    if ($count !== substr_count($rawHeaders, "\n")) {
        // Folding is deprecated, see https://tools.ietf.org/html/rfc7230#section-3.2.4
        if (preg_match(\DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Psr7\Rfc7230::HEADER_FOLD_REGEX, $rawHeaders)) {
 ...
 * @return null|string
 */
function get_message_body_summary(\DeliciousBrains\WP_Offload_Media\Gcp\Psr\Http\Message\MessageInterface $message, $truncateAt = 120)
{
    $body = $message->getBody();
