...
function promise_for($value)
{
    ...
    if (method_exists($value, 'then')) {
        ...
        $promise = new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\Promise($wfn, $cfn);
        $value->then([$promise, 'resolve'], [$promise, 'reject']);
        return $promise;
    }
    return new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\FulfilledPromise($value);
}
/**
...
function rejection_for($reason)
{
    ...
    return new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\RejectedPromise($reason);
}
/**
...
{
    return $reason instanceof \Exception || $reason instanceof \Throwable ? $reason : new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\RejectionException($reason);
}
/**
...
function some($count, $promises)
{
    $results = [];
    $rejections = [];
    return each($promises, function ($value, $idx, \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\PromiseInterface $p) use(&$results, $count) {
        if ($p->getState() !== \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\PromiseInterface::PENDING) {
            return;
        }
        $results[$idx] = $value;
...
{
    return (new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\EachPromise($iterable, ['fulfilled' => $onFulfilled, 'rejected' => $onRejected]))->promise();
}
/**
...
{
    return (new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\EachPromise($iterable, ['fulfilled' => $onFulfilled, 'rejected' => $onRejected, 'concurrency' => $concurrency]))->promise();
}
/**
...
{
    return each_limit($iterable, $concurrency, $onFulfilled, function ($reason, $idx, \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\PromiseInterface $aggregate) {
        $aggregate->reject($reason);
    });
}
/**
...
{
    return new \DeliciousBrains\WP_Offload_Media\Gcp\GuzzleHttp\Promise\Coroutine($generatorFn);
}
